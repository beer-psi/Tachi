...
const program = new Command().argument("<result.csv>").parse(process.argv);
...
for (let row = 1; row < csv.length - 2; row++) {
	let pairs: Partial<
		Record<Playtypes["arcaea"], Record<Difficulties[GPTStrings["arcaea"]], string>>
	>;
	let inGameStrID;

	if (header.length === 6) {
		const [igid, pst, prs, ftr, byd, etr] = csv[row] as [
			string,
			string,
			string,
			string,
	...
	for (const [playtype, difficulties] of Object.entries(pairs)) {
		for (const [difficulty, notecount] of Object.entries(difficulties)) {
			...
			if (!chart) {
				logger.warn(
					`Missing chart for inGameStrID ${inGameStrID} [${playtype} ${difficulty}]`
				);
				continue;
			}
			...
			if (chart.data.notecount !== 0 && chart.data.notecount !== Number(nc)) {
				logger.warn(
					`Chart ${inGameStrID} [${playtype} ${difficulty}] has a different notecount in database seeds. Did this chart change? OLD: ${chart.data.notecount} -> ${notecount}`
				);
				continue;
			}
