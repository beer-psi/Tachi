...
import { ARCAEA_CONTROLLER_IMPL, ARCAEA_TOUCH_IMPL } from "./arcaea";
...
import type { GPTStrings, ProvidedMetrics, ScoreData, ScoreDocument } from "tachi-common";
import type { DeepPartial } from "utils/types";

const baseMetrics: ProvidedMetrics[GPTStrings["arcaea"]] = {
	lamp: "CLEAR",
	score: 9_979_366,
};

const scoreData: ScoreData<GPTStrings["arcaea"]> = {
	lamp: "CLEAR",
	score: 9_979_366,
	grade: "EX+",
	judgements: {
...
for (const [playtype, impl] of [
	["Touch", ARCAEA_TOUCH_IMPL],
	["Controller", ARCAEA_CONTROLLER_IMPL],
] as const) {
	...
	t.test("Arcaea Implementation", (t) => {
		t.test("Grade Deriver", (t) => {
			const f = (score: number, expected: string) =>
				t.equal(
					impl.derivers.grade(dmf(baseMetrics, { score }), TestingArcaeaSheriruthFTR as any),
					expected,
		...
		t.test("Rating Calc", (t) => {
			t.equal(
				impl.scoreCalcs.potential(scoreData, TestingArcaeaSheriruthFTR as any),
				11.99,
		...
		t.test("Colour Deriver", (t) => {
			const f = (v: number | null, expected: any) =>
				t.equal(
					impl.classDerivers.badge({ naivePotential: v }),
					expected,
		...
		t.test("Goal Formatters", (t) => {
			t.test("Criteria", (t) => {
				t.equal(
					impl.goalCriteriaFormatters.score(10_002_221),
			...
			t.test("Out Of", (t) => {
				t.equal(impl.goalOutOfFormatters.score(10_001_003), "10,001,003");
				t.equal(impl.goalOutOfFormatters.score(9_983_132), "9,983,132");
		...
		t.test("Score Validations", (t) => {
			const f = (s: DeepPartial<ScoreDocument<GPTStrings["arcaea"]>>) =>
				RunValidators(impl.scoreValidators as any, dmf(mockScore, s));
			...
			t.strictSame(
				impl.chartSpecificValidators.score(
					...
					TestingArcaeaSheriruthFTR as any
				),
				true
			);
			...
			TestSnapshot(
				t,
				impl.chartSpecificValidators.score(-1, TestingArcaeaSheriruthFTR as any),
				"Arcaea IMPL Validators: Should disallow negative scores."
			);
			TestSnapshot(
				t,
				impl.chartSpecificValidators.score(10_001_152, TestingArcaeaSheriruthFTR as any),
